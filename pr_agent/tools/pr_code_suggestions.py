...
class PRCodeSuggestions:
    ...
    async def _prepare_prediction(self, model: str) -> dict:
        self.patches_diff = get_pr_diff(
            self.git_provider, self.token_handler, model, add_line_numbers_to_hunks=True, disable_extra_lines=True
        )
    ...
    def _prepare_pr_code_suggestions(self, predictions: str) -> Dict:
        data = load_yaml(
            predictions.strip(), keys_fix_yaml=["relevant_file", "suggestion_content", "existing_code", "improved_code"]
        )
        ...
        for i, suggestion in enumerate(data["code_suggestions"]):
            try:
                if (
                    not suggestion
                    or "one_sentence_summary" not in suggestion
                    or "label" not in suggestion
                    or "relevant_file" not in suggestion
                ):
    ...
    def generate_summarized_suggestions(self, data: Dict) -> str:
        try:
            ...
            if get_settings().pr_code_suggestions.self_reflect_on_suggestions:
                pr_body += (
                    f"""<thead><tr><td>Category</td><td align=left>{header}</td><td align=center>Score</td></tr>"""
                )
            else:
